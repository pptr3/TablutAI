	...
	public double getUtility(StateTablut state, Turn player) {
		...
		if(state.getCurrentDepth() == 0) {
			if(player.equals(Turn.WHITE)) {
				// add heuristic "whether in a certain position the black will winn with one move"
				toreturn = 100*(state.getDistanceFromKingToClosestEscapeArea()) - 2*state.isTheKingInTheThrone() + 2*state.getNumberCloseToTheKingOf(Pawn.WHITE) 
