...
public class StateTablut extends State implements Serializable {
	...
	public List<XYWho> getAllLegalMoves() {
		...
		if(this.getTurn().equals(Turn.WHITE)) {
			...
			if(super.hasTheKingMoved()) {
				for (int j = 0; j < currentBoardState.length; j++) {
					// (x, y - j) UP
					if(((king_position[1] - j) >= 0) && super.getPawn(king_position[0], king_position[1] - j) == Pawn.EMPTY && (super.getArea(king_position[0], king_position[1] - j) != Area.CAMPS) && (this.getArea(king_position[0], king_position[1] - j) != Area.CASTLE)) {
						whiteLegalMoves.add(new XYWho(king_position[0], king_position[1] - j, new int[]{king_position[0], king_position[1]}));
					}
					// (x, y + j) DOWN
					if(((king_position[1] + j) < currentBoardState.length) && super.getPawn(king_position[0], king_position[1] + j) == Pawn.EMPTY && (super.getArea(king_position[0], king_position[1] + j) != Area.CAMPS) && (super.getArea(king_position[0], king_position[1] + j) != Area.CASTLE)) {
						whiteLegalMoves.add(new XYWho(king_position[0], king_position[1] + j, new int[]{king_position[0], king_position[1]}));
					}
				}
				for (int i = 0; i < currentBoardState.length; i++) {
					// (x - i, y) LEFT
					if(((king_position[0] - i) >= 0) && super.getPawn(king_position[0] - i, king_position[1]) == Pawn.EMPTY && (super.getArea(king_position[0] - i, king_position[1]) != Area.CAMPS) && (this.getArea(king_position[0] - i, king_position[1]) != Area.CASTLE)) {
						whiteLegalMoves.add(new XYWho(king_position[0] - i, king_position[1], new int[]{king_position[0], king_position[1]}));
					}
					// (x + i, y) RIGHT
					if(((king_position[0] + i) < currentBoardState.length) && super.getPawn(king_position[0] + i, king_position[1]) == Pawn.EMPTY && (super.getArea(king_position[0] + i, king_position[1]) != Area.CAMPS) && (super.getArea(king_position[1] + i, king_position[1]) != Area.CASTLE)) {
						whiteLegalMoves.add(new XYWho(king_position[0] + i, king_position[1], new int[]{king_position[0], king_position[1]}));
					}
				}
				// if the king has already moved, since he cannot go in the CASTLE, remove the position (4, 4)
			}
